...
### 홈 화면
- **검색바**: 상단에 위치, 실시간 검색 지원
- **가장 인기있는 영화**: 대형 포스터 1개
- **현재 상영중**: 가로 스크롤 리스트
- **인기순**: 무한 스크롤 지원
- **평점 높은순**: 순위와 함께 표시
- **개봉예정**: 가로 스크롤 리스트
...
- **검색 결과**: 영화 포스터, 제목, 줄거리 미리보기
- **로딩 상태**: 검색 중 진행 표시기
- **빈 결과 처리**: 검색 결과가 없을 때 안내 메시지
- **검색 모드 전환**: 홈 화면과 검색 결과 화면 간 원활한 전환
...
**실시간 검색 시스템**의 핵심 구성 요소:

```dart
// 1. Debouncer로 검색 최적화
Debouncer(
  duration: Duration(milliseconds: 500),
  callback: () => _performSearch(),
)
...
void setSearchMode(bool isSearchMode, String query) {
  state = state.copyWith(
    ...
    searchQuery: query,
    searchResults: null,  // 이전 결과 초기화
    isSearching: false,
  );
}

// 3. TMDB Search API 활용
Future<MovieResponseDto?> searchMovies(String query) async {
  ...
  return MovieResponseDto.fromJson(jsonDecode(response.toString()));
}
```

**핵심 포인트:**
- **Debouncer**: 500ms 지연으로 불필요한 API 호출 방지
- **검색 모드**: 홈 화면과 검색 결과 화면 전환
- **실시간 결과**: 타이핑과 동시에 UI 업데이트
- **검색 취소**: X 버튼으로 검색 모드 해제
...
- `/movie/upcoming`: 개봉예정 영화
- `/movie/{id}`: 특정 영화 상세 정보
- `/search/movie`: 영화 검색
